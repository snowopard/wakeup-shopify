...
const Slider = async (section) => {
  ...
  Swiper.use([Pagination, Autoplay, EffectFade, EffectFlip, EffectCreative, A11y]);
  const selectors2 = {
    slider: ".js-slider",
    slide: ".swiper-slide",
    slideActive: ".swiper-slide.swiper-slide-active",
      ...
      if (currentVideo && currentVideo.length && isCurrentVideo) {
        currentVideo.forEach((curVideo) => {
          if (video.videoWrapper === curVideo && (JSON.parse(curVideo.dataset.autoplay) || initiatorClick)) {
            ...
            setTimeout(() => {
              const slideActive = section.querySelector(selectors2.slideActive);
  ...
  function handleVideoButtons(show, currentVideo) {
    ...
    if (currentVideo && currentVideo.length && !JSON.parse(currentVideo[0].dataset.autoplay) && pauseButton && playButton) {
    ...
    playButton == null ? void 0 : playButton.classList.remove(classes.active);
    if (show && pauseButton) {
      pauseButton.classList.add(classes.active);
      return;
    }
    pauseButton == null ? void 0 : pauseButton.classList.remove(classes.active);
  }
  function clickHandler(event, videos) {
    ...
    if (pauseButtonCurrent) {
      pauseButton == null ? void 0 : pauseButton.classList.remove(classes.active);
      playButton == null ? void 0 : playButton.classList.add(classes.active);
      pauseVideos(videos);
      return;
    }
    const playButtonCurrent = event.target.closest(selectors2.playButton);
    if (playButtonCurrent) {
      pauseButton == null ? void 0 : pauseButton.classList.add(classes.active);
      playButton == null ? void 0 : playButton.classList.remove(classes.active);
  ...
  function handleButtonSound(videos) {
    section.addEventListener("click", (e) => {
      ...
      if (!btnSound)
        return;
      const currentVideo = [...section.querySelectorAll(selectors2.currentVideo)].filter((video) => getComputedStyle(video).getPropertyValue("display") !== "none");
      const slideActive = btnSound.closest(selectors2.slideActive);
      if (!slideActive)
        return;
...
const selectors = {
  ...
  scrollDown: ".js-slideshow-scroll-down"
};
const Slideshow = () => {
  function onScrollDownClick(section) {
    const scrollDown = section.querySelector(selectors.scrollDown);
    const nextSection = section.closest(".shopify-section").nextElementSibling;
