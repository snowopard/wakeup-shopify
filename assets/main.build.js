...
const Accordion = (config = {}) => {
  ...
  function init() {
    containers = document.querySelectorAll(selectors$9.container);
    accordions = [];
    containers.forEach((container) => {
      accordions.push({
        container,
        controls: [...container.querySelectorAll(selectors$9.control)],
        content: [...container.querySelectorAll(selectors$9.content)],
        items: [...container.querySelectorAll(selectors$9.item)]
      });
    });
    setInnerHeight();
    setDefaultState();
  ...
  function setInnerHeight() {
    accordions.forEach(({ content }) => {
      content.forEach((item) => {
        if (!item.closest(selectors$9.item) || !item.closest(selectors$9.item).classList.contains(cssClasses2.active)) {
  ...
  function handleClickEvent(event) {
    const control = event.target.closest(selectors$9.control);
  ...
  function handleArrowEvents(event) {
    event.preventDefault();
    const container = event.target.closest(selectors$9.container);
  ...
  function expandItem(element, force) {
    if (!element) {
      return;
    }
    ...
    const elementContent = element.querySelector(selectors$9.content);
    animateContent(element, false);
    setTabIndexOnTarget(elementContent);
    element.classList.add(cssClasses2.active);
    element.querySelector(selectors$9.control).setAttribute("aria-expanded", true);
  }
  function animateContent(element, hide) {
    ...
    const elementContent = element.querySelector(selectors$9.content);
    const innerHeight = element.querySelector(selectors$9.inner).offsetHeight;
    if (hide) {
  ...
  function collapseItem(element, force) {
    ...
    element.querySelector(selectors$9.control).setAttribute("aria-expanded", false);
  }
  ...
  function collapseUnselectedItems(element) {
    const container = element.closest(selectors$9.container);
    const currentAccordion = accordions.find(
      ({ container: accordionContainer }) => container === accordionContainer
    );
    if (currentAccordion) {
  ...
  function toggleItem(control) {
    const item = control.closest(selectors$9.item);
    const container = control.closest(selectors$9.container);
    const isSingleOpen = (container == null ? void 0 : container.hasAttribute(attributes$4.singleOpen)) || settings.singleOpen;
    if (isSingleOpen) {
      collapseUnselectedItems(item);
    }
    if (isItemExpanded(item) && !isSingleOpen) {
      collapseItem(item, isSingleOpen);
      return;
    }
    expandItem(item, isSingleOpen);
  }
...
const cssClasses = {
  ...
  rtl: "rtl",
  imageMoveDisabled: "is-image-move-disabled"
};
const selectors$8 = {
  container: ".js-custom-dropdown",
  trigger: ".js-custom-dropdown-trigger",
  input: "input:checked",
  hasFocusVisible: ":has(:focus-visible)",
  notHaveFocus: ":not(:focus-within)"
};
const CustomDropdown = () => {
  function hasOpenedCustomDropdown() {
    return [...document.querySelectorAll(`${selectors$8.container}.${cssClasses.active}`)].length > 0;
  }
  function closeAllCustomDropdowns() {
    const containers = document.querySelectorAll(`${selectors$8.container}.${cssClasses.active}`);
    containers.forEach((container) => {
      container.classList.remove(cssClasses.active);
    });
  }
  function addContainerListeners() {
    const containers = document.querySelectorAll(selectors$8.container);
    const focusOutHandler = (container) => {
      window.setTimeout(() => {
        if (container.matches(selectors$8.notHaveFocus) && !document.activeElement.contains(container) && container.classList.contains(cssClasses.active)) {
    ...
    containers.forEach((container) => {
      ...
      newContainer.addEventListener("focusout", () => focusOutHandler(newContainer));
    });
  }
  function init() {
    const containerClickHandler = (event) => {
      const container = event.target.closest(selectors$8.container);
      container.classList.toggle(cssClasses.active);
      if (container.classList.contains(cssClasses.active)) {
        const containers = document.querySelectorAll(selectors$8.container);
        containers.forEach((currentContainer) => {
          if (currentContainer !== container) {
            currentContainer.classList.remove(cssClasses.active);
          }
        });
        const input = container.querySelector(selectors$8.input);
        if (!input) {
          return;
        }
        input.focus();
    ...
    const clickHandler = (event) => {
      const trigger = event.target.closest(selectors$8.trigger);
      const container = trigger == null ? void 0 : trigger.closest(selectors$8.container);
      if (container && !container.classList.contains(cssClasses.active)) {
        containerClickHandler(event);
        return;
      }
      if (event.pointerType || !event.target.closest(selectors$8.container)) {
        closeAllCustomDropdowns();
      }
    };
    const keyDownHandler = (event) => {
      if (event.shiftKey && event.key === "Tab" || event.key === "Escape" && hasOpenedCustomDropdown()) {
        closeAllCustomDropdowns();
      }
    };
...
const selectors$7 = {
  wrapper: ".js-image-mouse-move-wrapper",
  image: ".js-image-mouse-move"
};
const attributes$3 = {
  imageScale: "data-image-scale",
  animationSpeed: "data-animation-speed",
  moveDistance: "data-move-distance"
};
const ImageMouseMove = () => {
  ...
  function calculateMovement(event, section) {
    ...
    const moveX = clampPosition((xPercent - 0.5) * intensity, -25, 25);
    const moveY = clampPosition((yPercent - 0.5) * intensity, -25, 25);
    return { moveX, moveY };
  }
  ...
  function handleMouseMove(event, images2, sectionEl) {
    if (sectionEl.classList.contains(classes2.imageMoveDisabled)) {
      return;
    }
    const { moveX, moveY } = calculateMovement(event, sectionEl);
    images2.forEach((image) => {
      if (!image.classList.contains(classes2.imageMouseMove)) {
        return;
      }
      const isReverseTranslate = image.classList.contains(classes2.reverseTranslate);
      const computedStyle = window.getComputedStyle(image);
      const scaleAnimation = image.getAttribute(attributes$3.imageScale);
      const moveDistance = image.getAttribute(attributes$3.moveDistance);
      const animationSpeed = image.getAttribute(attributes$3.animationSpeed);
      let rotation = getRotation(computedStyle.transform);
      let top = isReverseTranslate ? -moveY : moveY;
      let left = isReverseTranslate ? -moveX : moveX;
      const translate = `translate(${moveDistance ? left * moveDistance : left}px, ${moveDistance ? top * moveDistance : top}px)`;
      const scale = scaleAnimation ? ` scale(${scaleAnimation})` : "";
      image.style.transition = `transform ${animationSpeed ? animationSpeed : 0.3}s linear`;
      image.style.transform = `${rotation}${translate}${scale}`;
    });
  }
  function init() {
    const wrappers = document.querySelectorAll(selectors$7.wrapper);
    wrappers.forEach((wrapper) => {
      const images2 = wrapper.querySelectorAll(selectors$7.image);
      wrapper.addEventListener("mousemove", (event) => handleMouseMove(event, images2, wrapper));
      wrapper.addEventListener("mouseleave", () => {
        images2.forEach((image) => {
          ...
          image.style.transform = `${rotation}translate(0px, 0px) scale(1)`;
        });
      });
    });
  }
...
const Popover = () => {
  ...
  function init() {
    document.addEventListener("click", (event) => {
      const button = event.target.closest(selectors$6.button);
      if (!button) {
        const contents = [...document.querySelectorAll(selectors$6.content)];
        contents.forEach((content2) => {
          content2.classList.remove(cssClasses2.active);
          const container2 = content2.closest(selectors$6.container);
          if (!container2) {
            return;
          }
          const button2 = container2.querySelector(selectors$6.button);
          if (!button2) {
            return;
          }
      ...
      const container = button.closest(selectors$6.container);
      if (!container) {
        return;
      }
      const content = container.querySelector(selectors$6.content);
      if (!content) {
        return;
      }
...
const Tabs = () => {
  function setUnderlinePosition(tab, underline) {
    if (!tab || !underline)
      return;
    const horizontalPadding = 16;
    const { offsetLeft, offsetWidth } = tab;
    underline.style.left = `${offsetLeft + horizontalPadding}px`;
    underline.style.width = `${offsetWidth - horizontalPadding * 2}px`;
    underline.style.transitionProperty = "left, width";
  }
  function init() {
    const tabsContainers = document.querySelectorAll("[data-tabs-container]");
    tabsContainers.forEach((tabsContainer) => {
      const underline = tabsContainer.querySelector(".js-tab-underline");
      const activeTab = tabsContainer.querySelector(".js-tab-button.is-active");
      setUnderlinePosition(activeTab, underline);
      document.addEventListener("click", (event) => {
        ...
        if (!tabsContainer.contains(button)) {
          return;
        }
        setUnderlinePosition(button, underline);
        const buttons = tabsContainer.querySelectorAll(".js-tab-button");
        ...
        const tabContent = tabsContainer == null ? void 0 : tabsContainer.querySelector(
          `[data-tab-content=${tabName}]`
        );
        const passiveTabs = tabsContainer == null ? void 0 : tabsContainer.querySelectorAll(
          "[data-tab-content].active"
        );
        const passiveTabsButtons = tabsContainer == null ? void 0 : tabsContainer.querySelectorAll("[data-tab].active");
        button.classList.add("active");
        button.setAttribute("aria-expanded", "true");
        tabContent == null ? void 0 : tabContent.classList.add("active");
...
const ProductCard = () => {
  ...
  function quickViewButtonHandler(event) {
    ...
    hasMultipleVariants ? emitQuickViewClickEvent(productHandle, variantId, isPromoBannerCard) : emitCartEvent(getVariantId(button), getMinQuantity(productCard), productCard);
  }
  ...
  function getClosestQuickViewButton(target) {
    return target.closest(selectors$5.quickViewButton);
  }
  function getClosestSwatch(target) {
    return target.closest(selectors$5.swatch);
  }
  function getClosestProductCard(element) {
    return element.closest(selectors$5.productCard);
  }
  function getClosestVideo(element) {
    return element.closest(selectors$5.video);
  }
  function getProductHandle(target) {
    return target.getAttribute(attributes$2.productHandle);
  }
  function getVariantId(target) {
    const currentImage = target.querySelector(`${selectors$5.imagesWrapper}.${cssClasses2.active}`);
  ...
  function toggleSwatch(swatch) {
    ...
    const swatches = [...productCard.querySelectorAll(selectors$5.swatch)];
    removeActiveClasses(swatches);
    setCurrentElementActive(swatch);
    toggleImage(productCard, swatchIndex);
    const productCardImage = productCard.querySelector(selectors$5.productCardImageWrapper);
    if (!productCardImage)
      return;
    productCardImage.style.display = "none";
  }
  function toggleImage(productCard, swatchIndex) {
    const images2 = [...productCard.querySelectorAll(selectors$5.imagesWrapper)];
  ...
  async function emitCartEvent(variantId, quantity, productCard) {
    try {
      const params = {
        id: variantId,
        quantity
  ...
  function getMinQuantity(target) {
    const minQuantityEl = target.querySelector(selectors$5.minQuantity);
    if (!minQuantityEl) {
      return 1;
    }
...
const QuickView = () => {
  ...
  const selectors2 = {
    ...
    formButton: ".js-quick-view-submit-button",
    ...
    status: ".js-quick-view-status",
    timer: ".js-quick-view .js-timer",
    submitText: ".js-quick-view-submit-text"
  };
  const attributes2 = {
  ...
  function onChangeVariant({ variant }) {
    ...
    const isCarousel = mediaLayout === "carousel";
    if (isCarousel || !isDesktop)
      return;
  ...
  function initQuickViewPopup() {
    ...
    formButtons = [...container.querySelectorAll(selectors2.formButton)];
    ...
    const mediaLayout = sliderEl == null ? void 0 : sliderEl.getAttribute(attributes2.mediaLayout);
    if (!hiddenSelect || !formButtons.length || !productLink || !price || !formError || !quantity || !quantityWrapper || !swatches) {
      return;
    }
    ...
    window.themeCore.CustomDropdown.addContainerListeners();
    const toggleConfig = {
  ...
  function getProductUrl(productHandle2, variant, templateSuffix, optionValues) {
    ...
    const url = new URL(`${window.location.origin}${window.themeCore.objects.routes.root_url}/products/${productHandle2}`);
  ...
  function updateSwatchLabelName(variant, container2) {
    const swatchNameElements = container2.querySelectorAll(".js-swatch-label-name");
  ...
  function updateButtons() {
    ...
    if (!currentVariant) {
      formButtons.forEach((button) => {
        const text = button.querySelector(selectors2.submitText);
        if (text) {
  ...
  function updateSku(variant) {
    const skuElement = document.querySelector(selectors2.sku);
    if (!skuElement) {
      return;
    }
  ...
  function updateStatus(variant) {
    ...
    const statusElement = document.querySelector(selectors2.status);
    if (!statusElement) {
      return;
    }
    statusElement.setAttribute(attributes2.status, status);
  }
  function handlerPauseVideo() {
    const pausedElements = [...document.querySelectorAll(selectors2.paused)];
  ...
  function initTimer() {
    const Timer2 = window.themeCore.utils.Timer;
    const timerEl = document.querySelector(selectors2.timer);
    if (!timerEl) {
      return;
    }
    Timer2(timerEl).init();
    timerEl.classList.add(cssClasses2.animated);
  }
  function initVideos() {
...
const selectors$4 = {
  ...
  button: ".js-localization-form-button",
  panel: ".js-localization-form-panel",
  list: ".js-localization-form-list",
  link: ".js-localization-form-link"
};
const classes = {
  hidden: "is-hidden",
  active: "is-active"
...
class localizationForm extends HTMLElement {
  constructor() {
    ...
    this.input = this.querySelector(selectors$4.input);
    this.button = this.querySelector(selectors$4.button);
    this.panel = this.querySelector(selectors$4.panel);
    this.list = this.querySelector(selectors$4.list);
    this.isActive = false;
    this.timeout = null;
    this.trapFocus = null;
  ...
  setEventListeners() {
    ...
    document.addEventListener("theme:all:loaded", () => {
      window.themeCore.EventBus.listen("Localization-Form:open", ({ component }) => component !== this && this.closeSelector());
      window.themeCore.EventBus.listen("Localization-Form:close-form", () => this.closeSelector());
      this.removeTrapFocus = window.themeCore.utils.removeTrapFocus;
      this.trapFocus = window.themeCore.utils.trapFocus;
      this.focusable = window.themeCore.utils.focusable;
    });
  }
  hidePanel() {
    var _a;
    clearTimeout(this.timeout);
    let links = this.panel.querySelectorAll(selectors$4.link);
    this.button.setAttribute(attributes$1.ariaExpanded, "false");
    this.panel.classList.remove(classes.active);
    links.forEach((link) => link.setAttribute("tabindex", -1));
    this.panel.style.height = `0px`;
    this.isActive = false;
    (_a = window.themeCore.EventBus) == null ? void 0 : _a.emit("Localization-Form:close", { component: this });
    this.removeFocusTarget(this.button);
    this.timeout = setTimeout(() => {
      this.list.scrollTop = 0;
      this.panel.classList.add(classes.hidden);
    }, 400);
  }
  ...
  onItemClick(event) {
    const listItems = event.target.closest(selectors$4.link);
    if (!listItems) {
      return;
    }
    event.preventDefault();
    this.input.value = listItems.dataset.value;
    const form = listItems.closest(selectors$4.form);
    if (!form) {
      return;
    }
    form.submit();
  ...
  openSelector() {
    var _a;
    (_a = window.themeCore.EventBus) == null ? void 0 : _a.emit("Localization-Form:open", { component: this });
    ...
    setTimeout(() => {
      clearTimeout(this.timeout);
      let links = this.panel.querySelectorAll(selectors$4.link);
      this.panel.classList.remove(classes.hidden);
      this.isActive = true;
      this.panel.style.height = `${Math.min(this.list.scrollHeight, 180)}px`;
      this.timeout = setTimeout(() => this.panel.classList.add(classes.active), 0);
      this.button.setAttribute(
        attributes$1.ariaExpanded,
        (this.button.getAttribute(attributes$1.ariaExpanded) === "false").toString()
      );
      this.focusTarget(this.panel);
  ...
  focusTarget(target) {
    ...
    const focusableElements = this.focusable(target);
    if (focusableElements.length) {
      window.setTimeout(() => this.trapFocus(target, { elementToFocus: focusableElements[0], preventScroll: true }), 0);
      return;
    }
    this.trapFocus(target);
  }
  removeFocusTarget(element = null) {
  ...
  isPanelActive() {
    return this.panel.classList.contains(classes.active);
  }
}
...
const Toggle = (config) => {
  ...
  const on2 = window.themeCore.utils.on;
  const off2 = window.themeCore.utils.off;
  ...
  function openToggleTarget(target) {
    ...
    window.themeCore.EventBus.emit(`Toggle:open`);
  ...
  function closeToggleTarget(target) {
    ...
    if (once && hasTransition(target)) {
      removeTargetAfterTransitionEnd(target);
    }
  ...
  function hasTransition(target) {
    ...
    const hasTransitionDuration = transitionDuration.split(",").some((d) => parseFloat(d) > 0);
    const hasTransitionDelay = transitionDelay.split(",").some((d) => parseFloat(d) > 0);
    return hasTransitionDuration || hasTransitionDelay;
  }
  function removeTargetAfterTransitionEnd(target) {
    on2("transitionend", target, transitionEnd);
    function transitionEnd(event) {
      if (event.target === target) {
        off2("transitionend", target, transitionEnd);
        target.remove();
      }
    }
  }
...
const Timer = (timerContainer) => {
  ...
  const selectors2 = {
    ...
    timerProgress: ".js-timer-progress",
    productTimer: ".js-product-countdown-timer",
    productTimerHeader: ".js-product-countdown-timer-header",
    announcementBarTextWrapper: ".js-announcement-bar-text-wrapper",
    announcementBarTextDesktop: ".js-announcement-bar-text-desktop",
    announcementBarTextMobile: ".js-announcement-bar-text-mobile"
  };
  const DATE_VALUES = {
    day: 1e3 * 60 * 60 * 24,
    hour: 1e3 * 60 * 60,
  ...
  let nodes = {};
  let timerProgressState = {
    previousSeconds: null,
    previousProgressOffset: null,
    previousTickTime: null,
    timerProgressCircumference: null
  };
  function getSettings() {
    try {
      return JSON.parse(timerContainer.querySelector(selectors2.settings).textContent);
    } catch {
      return null;
    }
  }
  function getNodes() {
    return {
      ...
      secondsDozens: timerContainer.querySelector(selectors2.secondsDozens),
      secondsUnits: timerContainer.querySelector(selectors2.secondsUnits),
      timerProgress: timerContainer.querySelector(selectors2.timerProgress)
    };
  }
  function getCountDownDate(settings2, startDate = null) {
    const { type, duration } = settings2;
    if (type === "duration") {
      const baseDate = startDate || getStartDate(settings2);
      return new Date(baseDate.getTime() + Number(duration) * DATE_VALUES.hour);
    }
    return getStartDate(settings2);
  }
  function setCountDownTimer(settings2) {
    let countDownDate = null;
    let interval = null;
    const checkTimer = () => {
      const now2 = Date.now();
      if (settings2.type === "end_date") {
        if (!countDownDate) {
          countDownDate = getCountDownDate(settings2);
        }
        const distance2 = countDownDate - now2;
        if (distance2 <= 0) {
          clearInterval(interval);
          return;
        }
        changeTimerMarkup(getDateToInner(distance2));
        changeTimerProgress(distance2);
        return;
      }
      const configuredStartDate = getStartDate(settings2);
      if (now2 < configuredStartDate.getTime()) {
        timerContainer.classList.add(classes2.hidden);
        const countdownWrapper2 = timerContainer.closest(selectors2.productTimer);
        const header2 = countdownWrapper2 ? countdownWrapper2.querySelector(selectors2.productTimerHeader) : null;
        if (header2)
          header2.classList.add(classes2.hidden);
        return;
      }
      const countdownWrapper = timerContainer.closest(classes2.productTimer);
      const header = countdownWrapper ? countdownWrapper.querySelector(classes2.productTimerHeader) : null;
      const announcementBarWrapper = timerContainer.closest(selectors2.announcementBarTextWrapper);
      const announcementBarTextDesktop = announcementBarWrapper ? announcementBarWrapper.querySelector(selectors2.announcementBarTextDesktop) : null;
      const announcementBarTextMobile = announcementBarWrapper ? announcementBarWrapper.querySelector(selectors2.announcementBarTextMobile) : null;
      if (announcementBarTextDesktop)
        announcementBarTextDesktop.classList.remove(classes2.hidden);
      if (announcementBarTextMobile)
        announcementBarTextMobile.classList.remove(classes2.hidden);
      if (header)
        header.classList.remove(classes2.hidden);
      timerContainer.classList.remove(classes2.hidden);
      if (!countDownDate) {
        countDownDate = getCountDownDate(settings2, configuredStartDate);
      }
      let distance = countDownDate - now2;
      if (distance <= 0) {
        if (settings2.loop && settings2.type === "duration") {
          const durationMs = Number(settings2.duration) * DATE_VALUES.hour;
          if (!durationMs) {
            changeTimerMarkup(getDateToInner(0));
            clearInterval(interval);
            return;
          }
          const configuredStartDate2 = getStartDate(settings2);
          const elapsed = now2 - configuredStartDate2.getTime();
          const completedCycles = Math.floor(elapsed / durationMs);
          countDownDate = new Date(configuredStartDate2.getTime() + (completedCycles + 1) * durationMs);
          distance = countDownDate - now2;
        } else {
          clearInterval(interval);
          return;
        }
      }
      changeTimerMarkup(getDateToInner(distance));
      changeTimerProgress(distance);
    };
    const now = Date.now();
    const initialCountDownDate = getCountDownDate(settings2);
    const initialDistance = initialCountDownDate - now;
    if (initialDistance > 0) {
      changeTimerProgress(initialDistance, true);
    }
    checkTimer();
    interval = setInterval(checkTimer, DATE_VALUES.second);
  }
  function getStartDate(settings2) {
    const { year, month, day, hour, minutes, timezone } = settings2;
    const timezoneDifference = Number(timezone);
    const finalHour = +hour - timezoneDifference;
    return new Date(Date.UTC(year, month, day, finalHour, minutes || 0));
  }
  function startTimerTickWithAnimation(settings2) {
    const countDownDate = getCountDownDate(settings2);
    ...
    const intervalId = setInterval(() => {
      ...
      if (currentMilliseconds >= targetMilliseconds) {
        ...
        changeTimerMarkup(finalDateToRender2, finalDateToRender2);
      }
  ...
  function setCountDownTimerProgress() {
    if (!nodes.timerProgress) {
      return;
    }
    const radius = parseInt(nodes.timerProgress.getAttribute("r"));
    timerProgressState.timerProgressCircumference = 2 * Math.PI * radius;
  }
  function changeTimerProgress(distance, firstInitialization) {
    if (!nodes.timerProgress) {
      return;
    }
    if (timerProgressState.previousTickTime === null) {
      timerProgressState.previousTickTime = performance.now();
    }
    const isTimerExpired = distance < 1e3;
    const seconds = (Math.floor(distance % DATE_VALUES.minute / DATE_VALUES.second) + 59) % 60;
    const progressOffset = timerProgressState.timerProgressCircumference * seconds / 60;
    const reset = timerProgressState.previousSeconds === 0 && seconds === 59;
    const tickTime = performance.now();
    const duration = tickTime - timerProgressState.previousTickTime;
    timerProgressState.previousTickTime = tickTime;
    if (reset) {
      nodes.timerProgress.animate(
        [{ strokeDashoffset: timerProgressState.previousProgressOffset }, { strokeDashoffset: timerProgressState.timerProgressCircumference }],
        {
          duration: 0,
          fill: "forwards",
          easing: "linear"
      ...
      timerProgressState.previousProgressOffset = timerProgressState.timerProgressCircumference;
    }
    if (isTimerExpired) {
      return;
    }
    nodes.timerProgress.animate(
      [{ strokeDashoffset: timerProgressState.previousProgressOffset }, { strokeDashoffset: progressOffset }],
      {
        duration: firstInitialization ? 0 : duration,
        fill: "forwards",
        easing: "linear"
      }
    );
    if (firstInitialization) {
      nodes.timerProgress.classList.add(classes2.active);
    }
    timerProgressState.previousProgressOffset = progressOffset;
    timerProgressState.previousSeconds = seconds;
  }
  ...
  function init() {
    if (settings) {
      if (settings.type === "duration" && !settings.loop) {
        const startDate = getStartDate(settings);
        const endTime = startDate.getTime() + Number(settings.duration) * DATE_VALUES.hour;
        if (Date.now() >= endTime) {
          const countdownWrapper = timerContainer.closest(selectors2.productTimer);
          if (countdownWrapper)
...
const QuantityWidget = (item, widgetConfig = {}) => {
  ...
  let widget = {};
  function init() {
    controls.input = item.querySelector(selectors$3.input);
    controls.decreaseBtn = item.querySelector(selectors$3.decrease);
    controls.increaseBtn = item.querySelector(selectors$3.increase);
  ...
  function onChangeQuantityClick(event) {
    const isIncrease = event.target.closest(selectors$3.increase);
    const isDecrease = event.target.closest(selectors$3.decrease);
...
const selectors$2 = {
  section: ".js-cart-upsell",
  slider: ".js-upsell-slider",
  navigationNextEl: ".js-upsell-slider-next",
  navigationPrevEl: ".js-upsell-slider-prev",
  pagination: ".js-cart-upsell-pagination",
  paginationWrapper: ".js-cart-upsell-pagination-wrapper"
};
const CartUpsell = () => {
  const cssClasses2 = window.themeCore.utils.cssClasses;
  let Swiper2 = window.themeCore.utils.Swiper;
  let sections = [];
  function init(sectionId) {
    sections = [...document.querySelectorAll(selectors$2.section)].filter(
      (section) => !sectionId || section.closest(`#shopify-section-${sectionId}`)
    );
    sections.forEach((section) => {
      let slider = section.querySelector(selectors$2.slider);
      if (!slider)
        return;
      let paginationWrapper2 = slider.querySelector(selectors$2.paginationWrapper);
      let sliderOptions = {
        grabCursor: true,
        slidesPerView: 2,
        speed: 800,
        pagination: {
          type: "progressbar",
          el: selectors$2.pagination
        },
        navigation: {
          nextEl: selectors$2.navigationNextEl,
          prevEl: selectors$2.navigationPrevEl
        },
        breakpoints: {
          991: {
            slidesPerView: 3
        ...
        on: {
          init: function(swiper) {
            if (swiper.isLocked && paginationWrapper2) {
              paginationWrapper2.classList.add(cssClasses2.hidden);
            }
          }
        }
      };
      let upsellSlider = new Swiper2(slider, sliderOptions);
      if (paginationWrapper2) {
        upsellSlider.on("lock", function() {
          paginationWrapper2.classList.add(cssClasses2.hidden);
        });
        upsellSlider.on("unlock", function() {
          paginationWrapper2.classList.remove(cssClasses2.hidden);
        });
      }
    });
  }
...
const DOMContentLoadedPromise = new Promise((resolve) => {
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    window.themeCore.CustomDropdown = window.themeCore.CustomDropdown || CustomDropdown();
    window.themeCore.ImageMouseMove = window.themeCore.ImageMouseMove || ImageMouseMove();
    ...
    window.themeCore.CustomDropdown.init();
    window.themeCore.ImageMouseMove.init();
    ...
    window.themeCore.EventBus.listen("accordion:init", window.themeCore.Accordion.init);
    resolve();
  });
});
(async () => {
  ...
  try {
    ...
    const removePageBlur = (type) => {
      const blurTargets = {
        ...
        "age-verifier-popup": ["ageVerifierPopupToggle"],
